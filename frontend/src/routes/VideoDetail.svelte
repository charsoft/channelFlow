<script lang="ts">
  ...
  $: {
    if (activeTab === 'copy' && videoData) {
        availableImages = videoData.image_urls || [];
  ...
  async function handleGenerateImage(prompt: string, index: number) {
      ...
      try {
          ...
          videoData.image_urls = [...(videoData.image_urls || []), newImage.image_url];
        ...
        <div class="tab-content active">
             <div class="detail-section">
                ...
                <div class="thumbnails-grid">
                    {#each videoData.image_urls || [] as imageUrl}
                        ...
                        {@const state = promptGenerationStates[index]}
                         <li class:generating={state?.isGenerating}>
                            ...
                            <div class="prompt-controls">
                                ...
                                <button class="button-outline generate-image-btn" on:click={() => handleGenerateImage(prompt, index)} disabled={state?.isGenerating}>
                                    {#if state?.isGenerating}
                                        <div class="loader-small-white"></div>
                                    {:else}
                                        Generate Image
                                    {/if}
                                </button>
